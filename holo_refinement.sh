...
#updated: 2021-01-20
#$ -cwd
source /wynton/home/rotation/cmoore/phenix-1.19-4092/phenix_env.sh
...
echo '________________________________________________________Running refinement with ligand.________________________________________________________'
    if grep -F _refln.F_meas_au $PDB-sf.cif; then
        phenix.refine $PDB.updated.pdb $PDB-sf.mtz ${PDB}.ligands.cif /wynton/home/rotation/cmoore/custom.params refinement.input.xray_data.labels="FOBS,SIGFOBS" refinement.input.xray_data.r_free_flags.generate=True
    else
        echo 'IOBS'   
        phenix.refine $PDB.updated.pdb $PDB-sf.mtz ${PDB}.ligands.cif /wynton/home/rotation/cmoore/custom.params  refinement.input.xray_data.r_free_flags.label=R-free-flags refinement.input.xray_data.labels="IOBS,SIGIOBS"
    fi
    ...
    if grep -F _refln.F_meas_au $PDB-sf.cif; then
        phenix.refine $PDB.updated.pdb $PDB-sf.mtz /wynton/home/rotation/cmoore/custom.params  refinement.input.xray_data.labels="FOBS,SIGFOBS" refinement.input.xray_data.r_free_flags.label=R-free-flags 
    else
        phenix.refine $PDB.updated.pdb $PDB-sf.mtz /wynton/home/rotation/cmoore/custom.params  refinement.input.xray_data.r_free_flags.label=R-free-flags refinement.input.xray_data.labels="IOBS,SIGIOBS"
   fi
 fi
...
echo '________________________________________________________Validating the Ligand after Initial Refinement________________________________________________________'
  ...
  #qsub /wynton/home/rotation/cmoore/grid_search_ens_refine.sh $PDB
